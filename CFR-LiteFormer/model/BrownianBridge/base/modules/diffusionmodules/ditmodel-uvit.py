...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.in_blocks = nn.ModuleList([
            DiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio,qkv_bias=False, qk_scale=None,
                norm_layer=nn.LayerNorm, use_checkpoint=False)
            for _ in range(depth // 2)])

        self.mid_block = DiTBlock(hidden_size,  num_heads, mlp_ratio=mlp_ratio,qkv_bias=False, qk_scale=None,
                norm_layer=nn.LayerNorm, use_checkpoint=False)

        self.out_blocks = nn.ModuleList([
            DiTBlock(hidden_size, num_heads, skip=True, mlp_ratio=mlp_ratio,qkv_bias=False, qk_scale=None,
                norm_layer=nn.LayerNorm, use_checkpoint=False)
            for _ in range(depth // 2)])
...
def DiT_uvit_2(**kwargs):
    return DiT(depth=13, hidden_size=512, patch_size=2, num_heads=8, **kwargs)
